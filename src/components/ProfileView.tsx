...
import { 
  ...
  Download, Maximize2, Star, Zap, Sparkles, Grid3x3
} from 'lucide-react';
import { Garment, Outfit, UserProfile, SavedRecommendation } from '../types';
...
export const ProfileView = ({ user, targetUserId, onUpgrade }: ProfileViewProps) => {
  ...
  const [activeTab, setActiveTab] = useState<'wardrobe' | 'outfits' | 'favorites' | 'lab'>('wardrobe');
  ...
  useEffect(() => {
    ...
    const unsubscribeProfile = onSnapshot(doc(db, 'users', uid), (docSnap) => {
      if (docSnap.exists()) {
        ...
        if (data.favorites && data.favorites.length > 0) {
          ...
          }, (error) => handleFirestoreError(error, OperationType.GET, 'outfits/favorites'));
        } else {
          setFavorites([]);
        }
      }
    ...
    const outfitsQuery = isOwner
      ? query(collection(db, 'outfits'), where('authorId', '==', uid), orderBy('createdAt', 'desc'))
      : query(collection(db, 'outfits'), where('authorId', '==', uid), where('isPublic', '==', true), orderBy('createdAt', 'desc'));
    const unsubscribeOutfits = onSnapshot(outfitsQuery, (snapshot) => {
      setUserOutfits(snapshot.docs.map(d => ({ ...d.data(), id: d.id } as Outfit)));
    }, (error) => handleFirestoreError(error, OperationType.GET, 'outfits'));

    const garmentsQuery = query(collection(db, 'garments'), where('ownerId', '==', uid), orderBy('createdAt', 'desc'));
    const unsubscribeGarments = onSnapshot(garmentsQuery, (snapshot) => {
      setUserGarments(snapshot.docs.map(d => ({ ...d.data(), id: d.id } as Garment)));
    }, (error) => handleFirestoreError(error, OperationType.GET, 'garments'));

    const recommendationsQuery = isOwner
      ? query(collection(db, 'saved_recommendations'), where('userId', '==', uid), orderBy('createdAt', 'desc'))
      : query(collection(db, 'saved_recommendations'), where('userId', '==', uid), where('isPublic', '==', true), orderBy('createdAt', 'desc'));
    const unsubscribeRecommendations = onSnapshot(recommendationsQuery, (snapshot) => {
      setSavedRecommendations(snapshot.docs.map(d => ({ ...d.data(), id: d.id } as SavedRecommendation)));
      setLoading(false);
    }, (error) => handleFirestoreError(error, OperationType.GET, 'saved_recommendations'));

    return () => {
      unsubscribeProfile(); unsubscribeOutfits(); unsubscribeGarments();
      unsubscribeRecommendations(); unsubscribeFollowers(); unsubscribeFollowing();
    };
  }, [uid, user.uid]);
