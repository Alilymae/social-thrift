        ...
        {loading ? (
          ...
          tutorials.map((tutorial) => (
            <motion.div 
              ...
              key={tutorial.id}
              onClick={() => setSelectedTutorial(tutorial)}
              className="bg-card rounded-3xl border border-primary/5 overflow-hidden hover:shadow-xl transition-all group cursor-pointer"
            >
              <div className="aspect-video bg-bg relative flex items-center justify-center group-hover:bg-primary/5 transition-colors">
                <Play className="text-primary opacity-50 group-hover:opacity-100 transition-opacity" size={48} />
                <div className="absolute bottom-4 left-4 right-4 flex gap-2 flex-wrap">
                  {tutorial.tags.map((tag, i) => (
                    <span key={`${tag}-${i}`} className="bg-primary/20 backdrop-blur-md text-primary text-[10px] uppercase tracking-widest px-2 py-1 rounded-lg">
                      {tag}
                    </span>
                  ))}
                </div>
              ...
              <div className="p-6 space-y-4">
                <div>
                  <h4 className="font-bold text-lg leading-tight text-text">{tutorial.title}</h4>
                  <p className="text-text/50 text-sm mt-1 line-clamp-2">{tutorial.description}</p>
                </div>
                <div className="flex items-center justify-between pt-2 border-t border-primary/5">
                  <div className="flex items-center gap-2">
                    <div className="w-8 h-8 bg-bg rounded-full flex items-center justify-center text-text/40">
                      <UserIcon size={16} />
                    </div>
                    <span className="text-xs font-medium text-text/70">{tutorial.authorName}</span>
                  </div>
                  <div className="flex items-center gap-3">
                    <button 
                      onClick={(e) => handleToggleLike(e, tutorial.id)}
                      className={`flex items-center gap-1 transition-colors ${
                        likedTutorials.includes(tutorial.id) ? 'text-red-500' : 'text-text/40 hover:text-red-500'
                      }`}
                    >
                      <Heart size={14} fill={likedTutorials.includes(tutorial.id) ? "currentColor" : "none"} />
                      <span className="text-[10px] font-bold">{tutorial.likesCount || 0}</span>
                    </button>
                    <div className="flex items-center gap-1 text-text/40">
                      <MessageSquare size={14} />
                      <span className="text-[10px] font-bold">{tutorial.commentsCount || 0}</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="col-span-full py-20 text-center text-text/40">
            ...
            <p>No tutorials found for "{searchTag}". Be the first to share one!</p>
          </div>
        )}
      </div>
